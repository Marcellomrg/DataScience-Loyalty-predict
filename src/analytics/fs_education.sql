--Cursos completos na plataforma de cursos(Qtde e quais) --DONE

--Cursos iniciados na plataforma de cursos(Qtde e quais) -- DONE

--Dias desde a ultima interação na plataforma de cursos

WITH tb_usuario_episodios_assistidos AS (

    SELECT   
            Idusuario,
            descSlugCurso,
            COUNT(descSlugCursoEpisodio) AS qntEpisodiosAssistidos
...
tb_cursos_episodios AS (
    ...
    SELECT 
            descSlugCurso,
            COUNT(descEpisodio) AS qntEpisodios
...
tb_pct_cursos AS (
    ...
    SELECT t1.idUsuario,
            t1.descSlugCurso,
            1. * t1.qntEpisodiosAssistidos / t2.qntEpisodios AS pctCursoCompleto

    FROM tb_usuario_episodios_assistidos AS t1

    LEFT JOIN tb_cursos_episodios AS t2
